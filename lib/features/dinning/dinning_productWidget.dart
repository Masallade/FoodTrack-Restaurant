...
class DinningProductWidget extends StatelessWidget {
  final Product product;
  final int index;
  final int length;
  final bool inRestaurant;
  final bool isCampaign;
  DinningProductWidget({super.key, required this.product, required this.index, required this.length, this.inRestaurant = false,
    this.isCampaign = false});
  DinningController _dinningController=Get.put(DinningController());
          ...
          Expanded(child: Padding(
            ...
            child: Row(children: [
              ...
              IconButton(
                onPressed: () {
                  if(Get.find<ProfileController>().profileModel!.restaurants![0].foodSection!) {
                    Get.find<RestaurantController>().getProductDetails(product.id!).then((itemDetails) {
                      if(itemDetails != null){
                        _dinningController.addElementToCart(itemDetails);
                      }
                    });
                  }else {
                ...
                icon: const Icon(CupertinoIcons.add_circled, color: Colors.deepOrange),
              ),
