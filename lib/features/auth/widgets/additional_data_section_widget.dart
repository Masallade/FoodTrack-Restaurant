          ...
          child: showTextField ? CustomTextFieldWidget(
            ...
            ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                shrinkWrap: true,
                itemBuilder: (context, i) {
                // If the index is out of range or no file is selected
                if (i >= authController.additionalList![index].length) {
                return const SizedBox();
                }

                FilePickerResult file = authController.additionalList![index][i];
                String fileName = file.files.single.path!.split('/').last;
                bool isImage = fileName.contains('jpg') || fileName.contains('jpeg') || fileName.contains('png');
                ...
                children: [
                Image.asset(
                fileName.contains('doc') ? Images.documentIcon : Images.pdfIcon,
                height: 20,
                width: 20,
                fit: BoxFit.contain,
                ),
                ...
                // Add file button
                if (authController.additionalList![index].isEmpty ||
                authController.additionalList![index].length <
                (authController.dataList![index].mediaData!.uploadMultipleFiles == 1 ? 6 : 1))
                InkWell(
                onTap: () async {
